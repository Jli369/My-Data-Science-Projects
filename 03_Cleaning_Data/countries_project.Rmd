...
un_df <- readr::read_csv("/cloud/project/03_Cleaning_Data/data/raw/country_profile_variables.csv")
```

```{r, solution = TRUE}
# Read in the dataset for literacy_df
literacy_df <- readr::read_csv("/cloud/project/03_Cleaning_Data/data/raw/cross-country-literacy-rates.csv")
```
...
```{r, solution = TRUE}
library(janitor)
un_df <- un_df %>% janitor::clean_names()
literacy_df <- literacy_df %>% janitor::clean_names()
```
...
library(tidyr)
library(dplyr)
literacy_df <- literacy_df %>% arrange(desc(year))
literacy_df <- literacy_df %>% distinct(entity, .keep_all = TRUE)
...
```{r, solution = TRUE}
un_df <- un_df %>% naniar::replace_with_na_all(condition = ~.x %in% c(-99, "-99", ".../..."))
```

Let's check to see if this did what we expected.

```{r, solution = TRUE}
#any(c(-99, "-99", ".../...") %in% un_df)
any(isTRUE("99" == un_df))
```
...
```{r, solution = TRUE}
un_df <- un_df %>% tidyr::separate(col = life_expectancy_at_birth_females_males_years, into = c("life_expectancy_at_birth_females", "life_expectancy_at_birth_males"), sep = "/")
```
...
class(un_df$life_expectancy_at_birth_females)

un_df$life_expectancy_at_birth_males <- as.numeric(un_df$life_expectancy_at_birth_males)

class(un_df$life_expectancy_at_birth_males)
...
```{r, solution = TRUE}
un_df$region <- as.factor(un_df$region)
```
...
#literacy_df <- literacy_df %>% rename(country = entity)
#inner_join(un_df, literacy_df, by = c(country) = entity)
```
...
```{r, solution = TRUE}
plotting_df <- countries_df %>% select(country, year, life_expectancy_at_birth_females, life_expectancy_at_birth_males, literacy_rates = literacy_rates_world_bank_cia_world_factbook_and_other_sources)
...
```{r, solution = TRUE}
plotting_df <- plotting_df %>% pivot_longer(cols = starts_with("life_expectancy_at_birth_"), names_to = "gender", values_to = "life_expectancy")
```
...
- We found out that there is a correlation between literacy and life expectancy, where people with higher life expectancy have higher literacy rates.

- How did we explore our questions?
- We explored our question by reading in a csv file of cross country literacy rates and a csv file of country profile variables. After doing this step, we stored the data in data frames where we proceeded to mutate them (by cleaning up the data and separating sections). We then combined pieces of data from both variables containing csv data to create a new data frame and plotted it to show us the relationship between literacy rates and life expectancy. We then separated the plot into 2 plots, males and females where we drew a line through each graph to make the relationship clearer. Lastly, we ran a correlation test and found that there is significance in our data as the p-value is less than 0.05.

- What did our explorations show us?
- Our explorations showed us that there is a strong positive relationship between life expectancy and literacy rates, where people with higher life expectancy demonstrates higher literacy rates.

- What follow up data science questions arise for you regarding this dataset now that we've explored it some?
- A question that arose after exploration of the data was whether there is a relationship between income and literacy rates.
