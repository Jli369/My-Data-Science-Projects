...
Is there a correlation between readers and the suggested price of a book on Leanpub?
...
We found out that there may be a potential correlation between number of readers and the suggested price of a book.
- How did we explore our questions?
We explored out quuestion by creating a spreadsheet and extracting data from Leanpub to add the spreadsheet. We then created a csv file from our spreadsheet and uploaded it to our data folder in this project. After doing so, we created a dataframe using the csv file and analyzed the dataframe through various means, such as plotting the data, running correlation tests, etc.
- What did our explorations show us?
From analyzing the data and running a correlation test, we see that the P-value of our correlation test is less than 0.05, meaning there is potenetial significance in the correlation between the data.
- What follow up data science questions arise for you regarding this dataset now that we've explored it some?
Is there a correlation between number of readers and the minimum price?
